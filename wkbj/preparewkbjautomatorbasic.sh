...
sed -i 's/ /,/g' test1_1
sed -i 's/ /,/g' test1_2
mv -f test1_1 ${date}.${name}_1
mv -f test1_2 ${date}.${name}_2
else
awk '{for (f = 1; f <= NF; f++) { a[NR, f] = $f }  } NF > nf { nf = NF } END {for (f = 1; f <= nf; f++) {for (r = 1; r <= NR; r++) {printf a[r, f] (r==NR ? RS : FS)} }}' test2  > test1
sed -i 's/ /,/g' test1
mv -f test1 ${date}.${name}
rm -fr test2 seisdetails.dat
fi

mv -f *card $wkbjdir/stations
mv -f *time $wkbjdir/times
mv -f *depth $wkbjdir/depths
mv -f ${date}.${name}.dat $wkbjdir/eventinfo
mv -f ${date}.${name} $wkbjdir/events
mv -f ${date}.${name}_* $wkbjdir/events
